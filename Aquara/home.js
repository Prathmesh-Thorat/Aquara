      ...
      window.addEventListener("scroll", () => {
        ...
        // Zoom and focus on different parts of the planet based on scroll position
        if (scrollProgress < 0.33) {
          // Focus on water (first third of the scroll)
          camera.position.z = 15 - scrollProgress * 15;
          camera.position.y = -scrollProgress * 2.5;
          planet.rotation.x = (-scrollProgress * Math.PI) / 4;
        } else if (scrollProgress < 0.66) {
          // Focus on atmosphere (second third of the scroll)
          const localProgress = (scrollProgress - 0.33) * 3;
          camera.position.z = 10 - localProgress * 1;
          camera.position.y = -2.5 + localProgress * 2.5;
          planet.rotation.x = -Math.PI / 4 + (localProgress * Math.PI) / 4;
        } else {
          // Focus on land (last third of the scroll)
          const localProgress = (scrollProgress - 0.66) * 3;
          camera.position.z = 9 + localProgress * 1;
          camera.position.y = localProgress * 2.5;
          planet.rotation.x = (localProgress * Math.PI) / 4;
        }

        // Update planet and related object positions
        planet.position.y = -scrollPosition * 0.0025;
